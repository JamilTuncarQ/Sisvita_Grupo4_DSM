...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupObservers()
        //Ver DB
        val viewEmotionsButton: Button = findViewById(R.id.viewEmotionsButton)
        viewEmotionsButton.setOnClickListener {
            val intent = Intent(this, ViewEmotionsActivity::class.java)
    ...
    private fun FaceDetector.setup() = run {
        setOnFaceDetectionListener(object : FaceDetector.OnFaceDetectionResultListener {
            override fun onSuccess(faceBounds: List<FaceBounds>, faceBitmaps: List<Bitmap>) {
                viewModel.onFacesDetected(this@MainActivity,faceBounds, faceBitmaps)
            }
        })
    }
